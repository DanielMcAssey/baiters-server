// License: MIT
// Source, Docs, Issues: https://github.com/cdanek/KaimiraWeightedList

using System.Collections;
using System.Text;
using static GLOKON.Baiters.Core.Utils.WeightErrorHandlingType;

namespace GLOKON.Baiters.Core.Utils
{
    /// <summary>
    ...
    public class WeightedList<T> : IEnumerable<T>
    {
        ...
        public T Next()
        {
            ...
            long nextProbability = _rand.NextInt64(_totalWeight);
            return (nextProbability < _probabilities[nextInt]) ? _list[nextInt] : _list[_alias[nextInt]];
        }

        public void AddWeightToAll(long weight)
        {
        ...
        public void SubtractWeightFromAll(long weight) => AddWeightToAll(weight * -1);

        public void SetWeightOfAll(long weight)
        {
        ...
        /// </summary>
        public long MinWeight => _minWeight;

        /// <summary>
        /// Maximum weight in the structure. 0 if Count == 0.
        /// </summary>
        public long MaxWeight => _maxWeight;
        ...
        public void Insert(int index, T item, long weight)
        {
        ...
        public void SetWeight(T item, long newWeight) => SetWeightAtIndex(IndexOf(item), FixWeight(newWeight));

        public long GetWeightOf(T item) => GetWeightAtIndex(IndexOf(item));

        public void SetWeightAtIndex(int index, long newWeight)
        {
            _weights[index] = FixWeight(newWeight);
            Recalculate();
        }

        public long GetWeightAtIndex(int index) => _weights[index];
        ...
        private void Recalculate()
        {
            ...
            List<long> scaledProbabilityNumerator = new(Count);
            List<int> small = new(Count); // STEP 2
            List<int> large = new(Count); // STEP 2
            foreach (long weight in _weights)
            {
                if (isFirst)
                {
            ...
            while (small.Count > 0 && large.Count > 0)
            {
                ...
                _alias[l] = g; // 5.4
                long tmp = scaledProbabilityNumerator[g] + scaledProbabilityNumerator[l] - _totalWeight; // 5.5, even though using ints for this algorithm is stable
...
#pragma warning restore S125 // Sections of code should not be commented out
        ...
        internal static long FixWeightSetToOne(long weight) => (weight <= 0) ? 1 : weight;

        // Throw an exception when adding a bad weight.
        internal static long FixWeightExceptionOnAdd(long weight) => (weight <= 0) ? throw new ArgumentException("Weight cannot be non-positive") : weight;

        private long FixWeight(long weight) => (BadWeightErrorHandling == ThrowExceptionOnAdd) ? FixWeightExceptionOnAdd(weight) : FixWeightSetToOne(weight);
    }
